...
CREATE OR REPLACE FUNCTION plani.f_planre_insert_empleados (
...
/**************************************************************************
 ...
 FUNCION: 		plani.f_planre_insert_empleados 
 DESCRIPCION:   inserta funcionario para la planilal de retroactivo mensual
 AUTOR: 		 (admin)
...
#25	ETR			01/08/2019			  RAC			registro de empleados para planilla de retroactivo mensual
***************************************************************************/
DECLARE
  v_registros			record;
  v_planilla			record;
...
BEGIN
    ...
    v_nombre_funcion = 'plani.f_planre_insert_empleados';
    ...
    for v_registros in execute('
              ...
              where  tc.codigo in (''PLA'', ''EVE'') 
                 and UOFUN.tipo = ''oficial'' and '
                || v_filtro_uo || ' uofun.fecha_asignacion <= ''' || v_planilla.fecha_fin || ''' 
                and (uofun.fecha_finalizacion is null or uofun.fecha_finalizacion >= ''' || v_planilla.fecha_ini || ''') 
