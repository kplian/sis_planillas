...
CREATE OR REPLACE FUNCTION plani.f_plareisu_insert_empleados (
...
BEGIN
       ...
       --#7 remueve codigo quemado que parece sin sentido ...  and uofun.id_uo = 9886
    
       v_query = 'select 
                  uofun.id_funcionario , 
                  array_agg(car.id_cargo) as cargos,
                  array_agg(car.id_escala_salarial) as escalas, 
                  array_agg(ofi.id_lugar) as lugares,
                ...
                where 
                       ( tc.codigo = ''PLA'' or (tc.codigo = ''EVE'')) 
                    and UOFUN.tipo = ''oficial'' 
                    and '|| v_filtro_uo || ' uofun.fecha_asignacion <= ''' || v_planilla.fecha_planilla || ''' 
                    and (uofun.fecha_finalizacion is null or uofun.fecha_finalizacion >= ''' || v_fecha_inicio || ''') 
                    ...
                    AND  uofun.id_funcionario not in (
                                                      ...
                                                      group by uofun.id_funcionario';
                                                  
                                        
                                                
        for v_registros in execute(v_query)loop
            ...
            inner join orga.tcargo car on car.id_cargo = uofun.id_cargo
            inner join orga.ttipo_contrato tipc on tipc.id_tipo_contrato = car.id_tipo_contrato --#7 recueprar configuracion de tipo contrato
            where 
                 ...
                 and  tipc.considerar_planilla = 'si';  --#7 configurable los contratos considerados
