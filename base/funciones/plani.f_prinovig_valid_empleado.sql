...
CREATE OR REPLACE FUNCTION plani.f_prinovig_valid_empleado (
...
$body$
  ...
  ***************************************************************************
   ...
   COMENTARIOS:
   AUTOR:    RAC KPLIAN
   DESCRIP:  funcion para vlaidar personal isnertado directamente en planilla de prima de personal vigente
   Fecha: 16-04-2020
 ...
 #113             26-04-2020       Rarteaga     KPLIAN    Creación
 ********************************************************************************/
DECLARE
  v_registros           record;
  v_planilla            record;
...
BEGIN
    ...
    v_nombre_funcion = 'plani.f_prinovig_valid_empleado';
    ...
    --obtiene datos de la planillas  de prevision de  prima
    ...
    if (v_existe = 'no') then
        raise exception 'No se puede añadir el funcionario a la planilla de prima de personal NO vigente';
    end if;
    return;
EXCEPTION
