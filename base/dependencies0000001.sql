...
CREATE OR REPLACE VIEW plani.nvdatos_horas_funcionario(
    id_funcionario,
    id_funcionario_planilla,
    nombre_col,
    valor_col)
         ...
         FROM (
                SELECT fp.id_funcionario,
                       ...
                       "substring"(fun.desc_funcionario2, 1, 58) AS
                        nombre_funcionario,
                       sum(s.total_normal) AS sum
                FROM asis.tmes_trabajo_det s
                     JOIN asis.tmes_trabajo mt ON mt.id_mes_trabajo =
                      s.id_mes_trabajo
                     JOIN plani.tfuncionario_planilla fp ON fp.id_funcionario =
                      mt.id_funcionario
                     JOIN orga.vfuncionario fun ON fun.id_funcionario =
                      fp.id_funcionario AND fun.id_funcionario =
                       mt.id_funcionario
                     JOIN plani.tplanilla plani ON plani.id_planilla =
                      fp.id_planilla AND plani.id_periodo = mt.id_periodo
                     JOIN plani.ttipo_planilla tp ON tp.id_tipo_planilla =
                      plani.id_tipo_planilla AND tp.codigo::text = 'PLASUE'
                       ::text
...
/***********************************I-DEP-MZM-PLANI-ETR-2503-12/01/2021****************************************/
CREATE OR REPLACE VIEW plani.nvdatos_func_planilla(
    id_funcionario,
    id_funcionario_planilla,
    nombre_col,
