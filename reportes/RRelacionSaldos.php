			...
			if($this->detalle[0]['periodo']>0){ 
				if ($this->objParam->getParametro('codigo_planilla')=='PLANRE' && $this->objParam->getParametro('consolidar')=='si'){//#ETR-3997
					if ($this->objParam->getParametro('personal_activo')=='activo'){
						$this->Cell(0,5,'Acumulado de ENERO a '.str_replace ('DE ','',$this->datos[0]['periodo']). ' (PERSONAL ACTIVO)',0,1,'C');
						
					}elseif($this->objParam->getParametro('personal_activo')=='retirado'){
						$this->Cell(0,5,'Acumulado de ENERO a '.str_replace ('DE ','',$this->datos[0]['periodo']). ' (PERSONAL RETIRADO)',0,1,'C');
					}else{
						$this->Cell(0,5,'Acumulado de ENERO a '.str_replace ('DE ','',$this->datos[0]['periodo']),0,1,'C');	
					}
		...
		$this->alto_header =$this->GetY(); 
			...
			if($this->detalle[0]['periodo']>0){//#84
				...
				if($this->objParam->getParametro('codigo_planilla')=='BONOVIG'){ //#127
					...
					$this->Cell(50+$ampliar,5,'Total','T',1,'C');
				}
				$this->SetX(30);
				$this->Cell(50+$ampliar,5,'Distrito','B',0,'C');  //
				...
				if($this->objParam->getParametro('codigo_planilla')=='PLAPRIVIG' or $this->objParam->getParametro('codigo_planilla')=='PLAPREPRI' or $this->objParam->getParametro('codigo_planilla')=='SPLAPREPRI' || $this->objParam->getParametro('codigo_planilla')=='SPLAPRIVIG' ){ //#125 #133
					...
					$this->Cell(50+$ampliar,5,'(Bs)','B',1,'C'); //#152
				}
				...
				for ($i=0; $i<count($this->datos);$i++){
					...
					if ($this->datos[$i]['tipo_pago']=='cheque' ){ //#147
						...
						if ($this->datos[$i]['tipo_pago']!=$this->tipo_pagoP){
							...
							if($this->detalle[0]['periodo']>0){//#158
								...
								if($this->objParam->getParametro('codigo_planilla')!='BONOVIG'){//#127
									$this->Cell(150+($ampliar*3),0.1,'','LRB',1,'L');
								}else{
					...
					if($ciudad!=$this->datos[$i]['nombre']){ 
						...
						if($this->detalle[0]['periodo']>0){//#84
							...
							if($this->objParam->getParametro('codigo_planilla')!='BONOVIG'){//#127
								$this->Cell(150+($ampliar*3),5,$this->datos[$i]['nombre'],'RLT',1,'L');
							}else{
					...
					if($this->detalle[0]['periodo']>0){//#84
						...
						$this->SetFont('','',8);
						$this->SetX(30);
						$this->Cell(50+$ampliar,5,'','L',0,'L');	
						$this->Cell(50+$ampliar,5,$this->datos[$i]['banco'],'',0,'L');	
						
						if($this->objParam->getParametro('codigo_planilla')!='BONOVIG'){//#127
						 	$this->Cell(50+$ampliar,5,number_format($this->datos[$i]['importe'],2,'.',','),'R',1,'R');//#80
						}else{
